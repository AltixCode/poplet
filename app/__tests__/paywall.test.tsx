...
describe("Paywall", () => {
  ...
  it("marks a benefit tile as acknowledged when tapped, purely as a local UI toggle", async () => {
    const { getByLabelText } = await renderWithProviders(<Paywall />);
    const tile = getByLabelText(t("feat1Title"));
    expect(tile.props.accessibilityState?.selected).toBe(false);
    await fireEvent.press(tile);
    expect(tile.props.accessibilityState?.selected).toBe(true);
    // Toggles back off, and never touches the purchase/restore flow.
    await fireEvent.press(tile);
    expect(tile.props.accessibilityState?.selected).toBe(false);
  });
});
